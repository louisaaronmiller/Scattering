...
def ScatterLength(l: int,E=1e-3,rmax = rmax,R=R, h = 0.001,flag = True,E_min = 0, E_max = 1):
    '''
    This function varies potential of the sphere
    '''
    V_0 = np.linspace(-30,-1,100) ##### MATCH----------------------
    if flag is False:
        E = np.linspace(E_min,E_max,500)
    delta = []
    k = []
...
plt.plot(V_0abs ,scattering_length,color = 'black',label = 'Numerical')
