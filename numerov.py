...
V_0 = 5 # -10e6 * 1.6e-19
E = 11111111111113 # should eventually become an array
rmax = 30
# taking hbar^2 / 2m == 1
...
def delta_second(l,rvals,kvals):
    deltavals = []
    k_0 = math.sqrt(E)
    for i in range(len(rvals)- 1):
        j_l_i = spherical_jn(l,k_0 * rvals[i])
        ...
        j_l_ip1 = spherical_jn(l,k_0 * (rvals[i] + k_0))
        n_l_ip1 = spherical_yn(l,k_0 * (rvals[i] + k_0))
...
def K2(rvals,uvals):
    ...
    K_array = []
    for i in range(len(rvals)-1):
        K = (rvals[i] * (uvals[i] + math.sqrt(E)))/((rvals[i] + math.sqrt(E)) * uvals[i])
        K_array.append(K)
    return K_array
...
phaseshift_vals2 = delta_second(l=0,rvals=r_aug,kvals = K2(r_aug,u_aug))
print(phaseshift_vals2)
